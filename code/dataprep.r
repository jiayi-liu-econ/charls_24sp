...
# house$hhexp[!house$hhexp %in% boxplot.stats(house$hhexp)$out] |> nrow() # try to drop outliers

house |> nrow()
house <- house |>
  drop_na()
fwrite(house, "./data/clean/house.csv")

data20 <- demo20 |> left_join(house, by = "householdID")
fwrite(data20, "./data/clean/data20.csv")
...
# fwrite(covid, "./data/clean/covid.csv")
...
data20 |> drop_na() |> nrow()
# obs count 17670
# obs count 11085 for no NA data
